...
class DashboardController extends Controller
{
    ...
    //         'subtitleFirst', 'valueFirst', 'valueSecond'
    //     ));
    // }
    ...
    public function index()
    {
        ...
        $comparison = '-6.8%'; // This can also come from the API if needed
        $titleSecond = 'Capturada';
        $revenue = isset($response['revenue']) ? $response['revenue'] : $default;
        $graphId = 'revenue-chart';
        $customers = isset($response['customers']) ? $response['customers'] : 0; // Default to 0 if not found
        $orders = isset($response['orders']) ? $response['orders'] : 0; // Default to 0 if not found
